...
// Run: dart run tool/generate_skills.dart
...
const flutterFrontendSkill = Skill(
...
Keep every Dart file under ~150 lines. When a screen would exceed that, extract focused, reusable widgets into `lib/features/<feature>/presentation/widgets/<descriptive_name>.dart` and import them from the screen file. Each extracted widget must:

* Have a single clear responsibility — name it after that responsibility (`PhoneInputField`, `SocialLoginButton`, `WelcomeHeader`, `CountryDropdown`).
* Be a public top-level class (no leading underscore) so other screens in the same feature can reuse it.
* Take its data through constructor parameters and surface user actions via callbacks (`onTap`, `onChanged`) — no business logic, no state-management calls inside.
* Use `AppTheme` tokens and `Theme.of(context).textTheme`, never hardcoded colors or fonts.
* Live in its own file: one widget per file, filename in `snake_case` matching the class name (`phone_input_field.dart` → `PhoneInputField`).

A 200-line `build` method is a code smell. Split it. Prefer many small, named widgets over one deep anonymous tree.
...
const Map<String, Skill> skillsByName = {
  'flutter-frontend': flutterFrontendSkill,
};
