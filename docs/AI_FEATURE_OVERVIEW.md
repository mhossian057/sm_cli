...
**Date:** 2026-06-10
**Audience:** CEO, Marketing Team, Prospective Clients
**Product:** `sm_cli` — Flutter project generator with AI-assisted scaffolding

---

## TL;DR (for the CEO)

We built an AI layer on top of our Flutter CLI that takes a client's design
files (Figma frames or screenshot folders) plus a one-paragraph brief, and
produces a **running Flutter app with the right features, the right theme,
and the right screens already wired up** — in minutes, not days.

It works with four leading AI vendors (Claude, OpenAI, Gemini, Grok), keeps
all design files traceable inside the project, and respects clean
architecture so the engineering team can keep building on top of it without
"AI mess" to clean up.
